document.addEventListener("DOMContentLoaded", function () {
  ...
  function createCard(data) {
    ...
    const respondedMyPosts =
      JSON.parse(sessionStorage.getItem("respondedMyPosts")) || [];

    const isInteracted =
      respondedPosts.includes(data.announcement_id) ||
  ...
  function filterPosts(filter) {
    ...
    cards.forEach((card) => {
      ...
      const gamesArray = Array.from(gameTagsElements).map((span) =>
        span.innerText.trim().toLowerCase(),
      );

      const ageFromValue =
        parseInt(
          card.querySelector("#ageFromTag").innerText.replace(/\D/g, ""),
        ) || 0;
      const ageToValue =
        parseInt(
          card.querySelector("#ageToTag").innerText.replace(/\D/g, ""),
        ) || 0;

      const microText = card.querySelector("#microTag").innerText.toLowerCase();
      const hasMicroInCard =
        microText.includes("есть") || microText.includes("да");

      const matchesGame =
        filterGamesLower.length === 0 ||
        gamesArray.some((game) => filterGamesLower.includes(game));

      const matchesAgeFrom =
        !filter.ageFrom || ageFromValue >= parseInt(filter.ageFrom);
      const matchesAgeTo =
        !filter.ageTo || ageToValue <= parseInt(filter.ageTo);
  ...
  async function loadAdvertisement() {
    try {
      ...
      if (response.ok) {
        ...
        const retrievedFilter = JSON.parse(
          sessionStorage.getItem("filterTags"),
        );
        // Проверка фильтра
        if (retrievedFilter) {
          filterPosts(retrievedFilter);
        }
    ...
    if (e.target.classList.contains("send-btn")) {
      ...
      try {
        const response = await apiRequestPost(
          `http://localhost:8000/a/${postId}/responses/new`,
          {},
          data,
        );
